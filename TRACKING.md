...
- `Purchase`: envio do formulário pelo comprador em **Confirmar meu pedido**, dentro da página de obrigado aberta por convite assinado pela loja. Envia valor informado em BRL, IDs dos produtos, quantidades e `eventID` estável por pedido.
- Correspondência avançada: nome e WhatsApp são normalizados no navegador e entregues ao Meta Pixel somente após autorização expressa. Não são enviados à API do site nem salvos pelo site.
...
1. Finalizar a compra no atendimento.
2. Informar ao assistente apenas uma referência interna do pedido. Não são necessários nome, telefone, e-mail, produtos ou valor.
3. Gerar o convite exclusivo e enviar ao comprador pelo WhatsApp. O comprador informa nome, WhatsApp, produtos, quantidades e o valor total pago, autoriza a medição e toca em **Confirmar meu pedido**. A loja não deve abrir nem confirmar links reais para teste.
...
- Os valores e produtos são declarados pelo comprador e não são confirmação bancária. A loja emite o convite somente após finalizar o pedido no WhatsApp e recebe o resumo de volta pelo WhatsApp.
