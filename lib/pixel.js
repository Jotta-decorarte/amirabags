...
export function initializePixel(customer = null) {
  ...
  if (initialized) {
    if (customer && !matchingApplied) {
      window.fbq("init", PIXEL_ID, normalizeCustomer(customer));
  ...
  matchingApplied = Boolean(customer);
  initialized = true;
  return true;
}
